...
function iterateZodObject({
  zodRef,
  useOutput
}: ParsingArgs<OpenApiZodAnyObject>): Record<string, SchemaObject> {
  return Object.keys(zodRef.shape as object).reduce(
    (carry, key) => ({
      ...carry,
      [key]: generateSchema(zodRef.shape[key] as OpenApiZodAny, useOutput)
    }),
  ...
  checks.forEach((item) => {
    switch (item.kind) {
      case "max":
        ...
        if (!item.inclusive) baseSchema.exclusiveMaximum = baseSchema.maximum;
        break;
      case "min":
        baseSchema.minimum = item.value;
        if (!item.inclusive) baseSchema.exclusiveMinimum = baseSchema.minimum;
        break;
      case "int":
...
>): SchemaObject {
  ...
  const requiredProperties = Object.keys(
    (zodRef as z.AnyZodObject).shape as object
  ).filter((key) => {
...
function parseBigInt({
  ...
  return merge(
    { type: "integer", format: "int64" } as SchemaObject,
    zodRef.description ? { description: zodRef.description } : {},
    ...schemas
  );
}
...
function parseDate({ zodRef, schemas }: ParsingArgs<z.ZodDate>): SchemaObject {
  return merge(
    { type: "string", format: "date-time" } as SchemaObject,
    zodRef.description ? { description: zodRef.description } : {},
    ...schemas
  );
}
...
function parseUnion({
  ...
  return merge(
    {
      oneOf: zodRef._def.options.map((schema) =>
