...
import { z } from "zod";
import { FirestoreProjectorStore } from "..";
...
describe("projector", () => {
  const db = FirestoreProjectorStore<Schema>(table);
  ...
  it("should update records by id", async () => {
    ...
    const result1 = await db.load(["User-1"]);
    expect(result1[0].state.age).toEqual(45);

    const result2 = await db.load(["User-3"]);
  ...
  it("should update records by filter", async () => {
    ...
    let result = await db.load(["User-1"]);
    ...
    result = await db.load(["User-2"]);
    expect(result[0].state.age).toEqual(1);
    expect(result[0].state.hireDate).toEqual(newHire);
    expect(result[0].state.managerName).toEqual(newManager);

    result = await db.load(["User-3"]);
    expect(result[0].state.age).not.toEqual(1);
    expect(result[0].state.hireDate).not.toEqual(newHire);
    expect(result[0].state.managerName).not.toEqual(newManager);
  });
